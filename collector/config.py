...
# 总量跟踪表文件
COLLECTOR_TOTAL_FILE = lambda: os.path.join(BASE_DIR, f"总量跟踪表-{datetime.now().strftime('%Y-%m-%d')}.xlsx")
# 返回情况表文件
COLLECTOR_RETURN_FILE = lambda: os.path.join(BASE_DIR, f"返回情况表-{datetime.now().strftime('%Y-%m-%d')}.xlsx")
# 所有厂商目标清单文件列表
